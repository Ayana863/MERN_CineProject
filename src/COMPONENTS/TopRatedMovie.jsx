    ...
    const fetchTopRatedMovies = async () => {
      try {
        ...
        const filteredMovies = moviesArray.filter(movie =>
          movie.category?.includes("toprated")
        )
        ...
        setTopRatedMovies(moviesWithRatings)
        ...
        <div ref={scrollRef} className="flex gap-x-6 px-6 overflow-hidden">
          ...
          {topRatedMovies.map(movie => {
            ...
            return (
              <div
                key={movie._id}
                className="relative min-w-[180px] bg-[#0b132b] rounded-xl shadow-xl hover:scale-105 transition"
              >
